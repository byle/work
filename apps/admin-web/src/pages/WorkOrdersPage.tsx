  ...
  return (
    <PageSection title="工单列表" description="查看工单类型、优先级、状态和关联项目，并进行执行人/审核人配置。">
      ...
      <div style={{ background: '#fff', border: '1px solid #e5e7eb', borderRadius: 12, padding: 16, marginBottom: 20, display: 'grid', gap: 12, gridTemplateColumns: '1fr 200px' }}>
        ...
        <FormSelect label="状态筛选" options={[{ label: '全部状态', value: '' }, { label: '待分配', value: 'pending_assign' }, { label: '执行中', value: 'in_progress' }, { label: '待审核', value: 'pending_review' }, { label: '已通过', value: 'approved' }, { label: '已驳回', value: 'rejected' }]} value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} />
      </div>
